...
현재 `img/` 폴더가 프로젝트 루트에 있음. Vite의 정적 파일 서빙을 위해 `packages/client/public/img/`로 이동이 필요하다.

```bash
# 이미지 이동 (루트 → client/public)
# img/ → packages/client/public/img/
```

이동 후 코드에서 참조: `/img/01-1.png` (절대 경로) 또는 `import` 문 없이 public URL 사용.
...
**원인:** 루트의 `img/` 폴더가 Vite 정적 파일 서빙 경로(`packages/client/public/`) 밖에 있음
**예방:** 배포 전 `packages/client/public/img/`로 이미지 이동. Vite의 `publicDir` 설정 또는 직접 이동.
...
export function HwatuCard({ frontSrc, backSrc = '/img/card_back.jpg', alt = '화투 카드', size = 'md', faceUp = true, onClick }: HwatuCardProps) {
...
| 이미지 파일 | img/ 폴더 21개 파일 | ✓ | 루트의 `img/` 폴더에 존재 확인 |
| public/ 폴더 | Vite 정적 파일 | ✗ | `packages/client/public/` 없음 — Wave 0에서 생성 필요 |
| 기존 Tailwind `h-dvh` | 모바일 레이아웃 | ✓ | Tailwind v4에서 지원 |

**누락 의존성 (fallback 없음):**
- `packages/client/public/` 폴더 없음 → Wave 0에서 생성 + 이미지 복사/이동 필수
...
- [ ] `packages/client/public/img/` — 이미지 파일 이동 (기능이 아닌 파일 복사)
