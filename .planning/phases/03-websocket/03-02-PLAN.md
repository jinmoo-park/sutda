...
<task type="auto">
  ...
  <files>packages/server/src/integration.test.ts, packages/server/package.json</files>
  <read_first>
...
**2) `packages/server/src/integration.test.ts` 생성:**
...
- **해결 방법:** index.ts 하단의 `httpServer.listen(PORT, ...)` 를 조건부로 변경:
  ```typescript
  // index.ts 하단 수정
