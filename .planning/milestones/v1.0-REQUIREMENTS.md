# Requirements Archive: v1.0 온라인 섯다 MVP
...
- [x] **INFRA-02**: 플레이어가 링크로 접속하면 닉네임 입력만으로 방에 참여할 수 있다 (회원가입 없음)
...
- [x] **INFRA-05**: 플레이어가 연결이 끊겨도 같은 링크로 재접속하면 게임에 복귀할 수 있다
...
- [x] **UI-02**: 게임 테이블 화면에 원형으로 플레이어 자리, 카드, 칩 잔액이 표시된다
- [x] **UI-03**: 자신의 카드는 앞면으로, 타인의 카드는 뒷면(또는 모드별 규칙)으로 표시된다
- [x] **UI-04**: 베팅 액션 버튼(콜/레이즈/다이)과 레이즈 금액 입력 필드가 표시된다
- [x] **UI-05**: 현재 판돈(팟) 금액이 테이블 중앙에 표시된다
