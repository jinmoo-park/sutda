...
**Goal**: 배포 전 사용자 경험을 완성한다 — 실제 카드 이미지, 테이블 배경, 카드 뒤집기 인터랙션, 텍스트 채팅, 베팅 강조 표시, 올인 POT 처리, 게임 이력 기능이 모두 완성된다.
**Depends on**: Phase 9
**Requirements**:
- UX-02: 텍스트 채팅 기능 구현
- UX-03: 카드 뒤집기 인터랙션 (card-flip.md 참고)
- UX-05: 카드 배분 시 즉시 노출하지 않고 사용자가 클릭/스와이프하여 뒤집어야 패 확인 처리 (패받기 모달의 확인 버튼 대체)
...
- ALLIN-POT: 올인 승리 시 POT 처리 — 올인 승리자는 자신의 잔액총액만큼만 각 플레이어에게서 받고, 차액은 원래 플레이어들에게 반환. 올인 패배 시 구경/재충전. 올인 상태에서는 베팅 패널 비활성화, 나머지 플레이어는 베팅 계속.
**Success Criteria** (완료 시 참이어야 하는 것):
  1. 카드가 실제 이미지로 표시되며 CSS 이펙트로 실물감이 있다
  2. 카드 배분 후 클릭/스와이프로 패를 뒤집어야 확인 처리된다 (확인 버튼 없음)
  3. 텍스트 채팅이 작동한다
  4. 내 베팅 차례일 때 베팅 패널이 강조 표시된다
  5. 올인 상황에서 POT이 올바르게 분배된다
  6. 게임 이력이 표시된다
**Plans**: TBD
...
**실행 순서:**
Phase 1 -> 2, 3 (병렬 가능) -> 4 -> 5 -> 6 -> 7 -> 8 -> 9 -> 10 -> 11
