...
- [ ] 세장섯다 모드: betting-2 시작 시 딜러가 올인 상태여도 프리즈 없이 진행
- [ ] 세장섯다 전체 플로우 정상 동작 확인 (패 분배 → 1차베팅 → 오픈 → 선택 → 2차베팅 → 결과)
...
- [ ] 참여자(직접 URL 입장) → 게임 중 재접속 시 닉네임 폼 없이 게임 복원 확인
...
- [ ] 섞기 담당자 재접속 후 ShuffleModal 정상 표시 확인
- [ ] 기리 담당자 재접속 후 CutModal 정상 표시 확인
...
- [ ] 잔액 이하로만 레이즈 금액 누적되는지 확인
- [ ] 잔액 딱 맞춰지면 "올인" 표시 + 칩버튼 비활성화 확인
...
- [ ] 0원 플레이어가 다음판 투표 시 강제퇴장 + "칩이 부족하여 퇴장되었습니다" 토스트 확인
- [ ] 퇴장 후 같은 방 닉네임 입력 폼으로 이동 확인
- [ ] 재입장(동일 닉네임) 시 observer로 정상 합류 확인
- [ ] 1명 남을 경우 대기실로 복귀 확인
...
- [ ] 올인 승리 시 승자에게 pot 전액 지급되는지 확인 (배포 후 테스트)
