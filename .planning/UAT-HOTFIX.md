...
### 21. 모바일 채팅 오버레이 idle 투명도 버그

**원인**: `useState(초기값)` 변경은 첫 렌더에만 적용. `handleBlur`, `triggerMobileFade` 타이머가 `0.05`를 하드코딩해서 어떤 초기값을 써도 idle 상태는 항상 5% 밝기로 고정됨.

**수정**: `IDLE_OPACITY = 0.25` 상수를 도입하고 하드코딩된 `0.05` 세 곳 모두 교체.

**파일**: `packages/client/src/components/layout/ChatPanel.tsx`

- [x] idle 상태에서 채팅 오버레이 투명도 0.25로 표시됨 확인
- [x] 메시지 수신 → 0.5로 올라갔다가 3초 후 0.25로 내려오는 동작 확인
- [x] 포커스 → 0.8, blur → 1초 후 0.25 동작 확인
...
## UAT 중 추가 수정사항 (2026-04-04 세션 5)

### 26. 모바일 기리(CutModal) 꾹누름 시 개체선택 방지

**버그**: 모바일에서 카드 더미를 꾹 누르는 과정에서 브라우저의 긴 눌림(long-press) 제스처가 발동해 텍스트/이미지 선택 메뉴가 표시됨.

**원인**: 드래그 영역에 `touch-action: none`은 있었으나 `user-select: none`이 없어서 브라우저 선택 이벤트가 통과됨.

**수정**: `CutModal.tsx` 스와이프 영역 div에 `userSelect: 'none'`, `WebkitUserSelect: 'none'` 추가.

- [x] 모바일 기리 중 꾹 눌러도 개체선택 메뉴 표시 안 됨 확인
