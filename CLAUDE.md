...
- 사용자는 **Windows** 사용자다. PowerShell/CMD 환경이므로 `bash` 명령어를 직접 실행할 수 없다.
- 사용자에게 명령어를 안내할 때는 Claude Code가 대신 실행하거나, 실행 불가 시 PowerShell 문법으로 안내한다.
- `bash scripts/pull-debug.sh` 같은 bash 명령은 Claude Code가 Bash 툴로 직접 실행한다.
